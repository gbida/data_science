...
def startchs(player: str) -> tuple:
    ...
    if player:
        testfile = []
        wordfile = random.choice(os.listdir('../dic/word/'))
...
# 랭킹 확인
def highscore():
    # test_main_rank.xlsx 열기, 시간
    highscore=pd.read_excel('./test_main_rank.xlsx', engine='openpyxl', index_col=0, usecols=[0, 1, 2, 3])
    return highscore.loc[1:10]  # 10등까지만 보여주기

# 게임 종료 후 결과 저장
def save_result(player):
    data=pd.read_excel('./test_main_rank.xlsx', engine='openpyxl', index_col=0)  # 랭킹 엑셀 읽기
    ...
    data.to_excel('./test_main_rank.xlsx')  # rank.xlsx에 덮어쓰기